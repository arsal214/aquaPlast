...
class TestimonialController extends BaseController
{
    ...
    public function store(Request $request): RedirectResponse
    {
        try {
            ...
            $data['icon'] = $request->hasFile('icon') ? $this->uploadFile($request->file('icon'), 'testimonials/icons') : 'https://png.pngtree.com/element_our/20200610/ourmid/pngtree-character-default-avatar-image_2237203.jpg';
    ...
    public function update(Request $request, string $id): RedirectResponse
    {
        try {
            ...
            if ($request->hasFile('icon')) {
                $data['icon'] = $this->uploadFile($request->file('icon'), 'testimonials/icons');
            }
