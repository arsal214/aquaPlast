            ...
            <ul class="nav nav-sidebar" data-nav-type="accordion">
                ...
                @canany(['users-list'])
                    <x-nav-item active="users.*" icon="users-three" title="Users" :submenu="true">
                        @can('users-list')
                            <x-nav-item route="users.staff.index" active="users.staff.*" title="Admins" />
                        @endcan
                ...
                @canany(['productCategory-list'])
                    <x-nav-item active="catalog.*" icon="notebook" title="Catalog" :submenu="true">
                        @can('productCategory-list')
                            <x-nav-item route="catalog.category.index" active="catalog.category.*" title="Category" />
                        @endcan
                    </x-nav-item>
                ...
                @canany(['blogs-list', 'aboutUs-list', 'testimonials-list', 'homepage-list'])
                    <x-nav-item active="pages.*" icon="house" title="Pages" :submenu="true">
                        @can('homepage-list')
                            <x-nav-item route="pages.homepage.index" active="pages.homepage.*" title="HomePage" />
                        @endcan
                        @can('blogs-list')
                            <x-nav-item route="pages.blogs.index" active="pages.blogs.*" title="Blogs" />
                        @endcan

                        @can('aboutUs-list')
                            <x-nav-item route="pages.about-us.index" active="pages.about-us.*" title="About Us" />
                        @endcan

                        <x-nav-item route="pages.term-conditions.index" active="pages.term-conditions.*"
                            title="Term Conditions" />

                        <x-nav-item route="pages.teams.index" active="pages.teams.*" title="Teams" />

                        <x-nav-item route="pages.privacy-policy.index" active="pages.privacy-policy.*"
                            title="Privacy Policy" />

                        <x-nav-item route="pages.contact-us.index" active="pages.contact-us.*" title="Contact Us" />

                        <x-nav-item route="pages.testimonials.index" active="pages.testimonials.*" title="Testimonials" />


                        <x-nav-item active="pages.product-slider.*" route="pages.product-slider.index"
                    active="pages.product-slider.*" title="Product Slider" />
